...
class Damdfe extends DaCommon
{
    ...
    private function headerMDFePaisagem($x, $y)
    {
        ...
        if ($this->flagDocs) {
            ...
            $this->pdf->textBox($x, $y, 12, 8, 'Página ' . $this->pdf->PageNo() . '/{nb}', $aFont, 'T', 0, 0);
        }
        return $oldY + 8;
    }
    ...
    private function headerMDFeRetrato($x, $y)
    {
        ...
        if (!empty($this->logomarca)) {
            ...
            if ($this->logoAlign == 'L') {
                // ajusta a dimensão do logo
                $nImgW = round((round($maxW * 0.35, 0)) / 3, 0);
                $nImgH = round(($h - $y) - 2, 0) + $y;
                $xImg = $x + 1;
                $yImg = round(($h - $nImgH) / 2, 0) + $y;
                //estabelecer posições do texto
        ...
        $this->pdf->textBox(
            ...
            $w,
            8,
            '- Documento Auxiliar de Manifesto Eletronico de Documentos Fiscais',
            $aFont,
            'T',
            'L',
            0,
    ...
    private function bodyMDFe($x, $y)
    {
        ...
        $this->pdf->textBox($x1 - 1, $y, $x2 - 20, 10, '', $this->baseFont, 'T', 'L', 0, '', 0, 0, 0, 1);
        $texto = 'Série';
        $aFont = array('font' => $this->fontePadrao, 'size' => 8, 'style' => '');
        ...
        $texto = !empty($this->ide->getElementsByTagName("dhEmi")->item(0)->nodeValue) ? date('d/m/Y H:i:s', strtotime($this->getTagValue($this->ide, "dhEmi"))) : '';
        $aFont = array('font' => $this->fontePadrao, 'size' => 10, 'style' => 'B');
        ...
        //o aquaviario tambem imprime o peso total, que antes ficava em bloco proprio
        if (
            $this->rodo
            || $this->aereo
            || $this->ferrov
        ...
        $this->pdf->code128($x1 + 1, $y, $this->chMDFe, ($maxW / 2) - 10, $bH);
        $this->pdf->setFillColor(255, 255, 255);
        ...
        //o CIOT e impresso no bloco do modal rodoviario, com numero e responsavel
        ...
        if ($this->rodo) {
            ...
            $aFont = array('font' => $this->fontePadrao, 'size' => 7, 'style' => '');
            $this->pdf->textBox($x1, $y, $x2, 8, $texto, $aFont, 'T', 'L', 0, '', false);
            $this->pdf->line($x1, $y + 4, $x1 + 96, $y + 4);
        ...
        $x1 = round($maxW / 2, 0) + 7;
        $x2 = ($maxW / 6);
        $this->quantidadeChavesLayout = 18;
        if ($this->orientacao == 'L') {
            $x1 = 225;
            $y = $yold - 5;
